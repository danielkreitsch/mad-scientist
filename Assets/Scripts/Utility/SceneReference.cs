...
namespace Utility
{
    ...
    public class SceneReferencePropertyDrawer : PropertyDrawer
    {
        ...
        private static readonly float paddedLine = SceneReferencePropertyDrawer.lineHeight + SceneReferencePropertyDrawer.PAD_SIZE;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            EditorGUI.BeginProperty(position, GUIContent.none, property);
            {
                ...
                property.isExpanded = EditorGUI.Foldout(new Rect(position.x, position.y, position.width, SceneReferencePropertyDrawer.lineHeight), property.isExpanded, label);
                ...
                if (property.isExpanded)
                {
                    ...
                    position.height -= SceneReferencePropertyDrawer.lineHeight;
                    position.y += SceneReferencePropertyDrawer.lineHeight;

                    var sceneAssetProperty = SceneReferencePropertyDrawer.GetSceneAssetProperty(property);

                    // Draw the Box Background
                    position.height -= SceneReferencePropertyDrawer.FOOTER_HEIGHT;
                    GUI.Box(EditorGUI.IndentedRect(position), GUIContent.none, EditorStyles.helpBox);
                    position = SceneReferencePropertyDrawer.boxPadding.Remove(position);
                    position.height = SceneReferencePropertyDrawer.lineHeight;
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        if (buildScene.scene == null) SceneReferencePropertyDrawer.GetScenePathProperty(property).stringValue = string.Empty;
                    }

                    position.y += SceneReferencePropertyDrawer.paddedLine;
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            return SceneReferencePropertyDrawer.boxPadding.vertical + SceneReferencePropertyDrawer.lineHeight * lines + SceneReferencePropertyDrawer.PAD_SIZE * (lines - 1) + SceneReferencePropertyDrawer.FOOTER_HEIGHT;
        }
        ...
        private void DrawSceneInfoGUI(Rect position, BuildUtils.BuildScene buildScene, int sceneControlID)
        {
            ...
            using (new EditorGUI.DisabledScope(readOnly))
            {
                ...
                iconRect.width = iconContent.image.width + SceneReferencePropertyDrawer.PAD_SIZE;
        ...
        {
            return property.FindPropertyRelative(SceneReferencePropertyDrawer.sceneAssetPropertyString);
        }

        private static SerializedProperty GetScenePathProperty(SerializedProperty property)
        {
            return property.FindPropertyRelative(SceneReferencePropertyDrawer.scenePathPropertyString);
        }
        ...
        private static class DrawUtils
        {
            ...
            {
                position.width = EditorGUIUtility.labelWidth - SceneReferencePropertyDrawer.PAD_SIZE;
                return position;
            }
        }
        ...
        private static class BuildUtils
        {
            ...
            public static bool IsReadOnly()
            {
                ...
                var timeSinceLastCheck = curTime - BuildUtils.lastTimeChecked;

                if (!(timeSinceLastCheck > BuildUtils.minCheckWait)) return BuildUtils.cachedReadonlyVal;

                BuildUtils.lastTimeChecked = curTime;
                BuildUtils.cachedReadonlyVal = BuildUtils.QueryBuildSettingsStatus();

                return BuildUtils.cachedReadonlyVal;
            }
